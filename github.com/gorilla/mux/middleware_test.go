...
func TestCORSMethodMiddlewareSubrouter(t *testing.T) {
	...
	subrouter.HandleFunc("/main", stringHandler("a")).Methods(http.MethodGet, http.MethodOptions, http.MethodPost)
	subrouter.HandleFunc("/main/{name}", stringHandler("b")).Methods(http.MethodGet, http.MethodOptions)

	subrouter.Use(CORSMethodMiddleware(subrouter))

	rw := NewRecorder()
	req := newRequest("GET", "/test/main/asdf")
	router.ServeHTTP(rw, req)
